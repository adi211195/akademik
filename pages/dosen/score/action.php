...
$score_file_type	=htmlspecialchars(@$_POST['score_file_type']);
...
switch ($action) {		
        ...
        $folderUpload = "../../../assets/score_file";
		...
		$score_file = $acak.$_FILES['score_file']['name'];
		$x = explode('.', $score_file);
		$ekstensi = strtolower(end($x));
		$ukuran	= $_FILES['score_file']['size'];
		$file_tmp = $_FILES['score_file']['tmp_name'];	
			if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
			    if($ukuran < 1044070){		
			    	$delete=mysqli_query($connect,"DELETE FROM master_score_file where 
			    		curriculum_id='$curriculum_id' AND 
			    		score_file_type='$score_file_type'");
			    	if (!empty($file)) {
			    		unlink("../../../assets/score_file/".$file);
			    	}
			    		
					move_uploaded_file($file_tmp, $folderUpload.'/'.$score_file);
        			$save=mysqli_query($connect,"INSERT INTO master_score_file
                      (score_file_id,
                      curriculum_id,
                      score_file,
                      score_file_type,
