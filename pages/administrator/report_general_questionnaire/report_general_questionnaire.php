			<div id="content-container">
                <div id="page-head">
                    ...
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <div id="page-title">
                        <h1 class="page-header text-overflow">General Questionnaire Reports</h1>
                    </div>
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <!--End page title-->
					...
					<li class="active">General Questionnaire Reports</li>
                    </ol>
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <!--End breadcrumb-->
                ...
                $check=mysqli_num_rows(mysqli_query($connect, "SELECT * FROM questionnaire_report_general
											where school_year='$school_year'
											AND semester='$semester'
											AND qr_status='finish'"));

                $page_input="page.php?p=report_general_questionnaire&act=input";
                $page_edit="page.php?p=report_general_questionnaire&act=edit";
                $back="page.php?p=report_general_questionnaire";
                $page_print="pages/administrator/report_general_questionnaire/report_pdf.php";
                $action="pages/administrator/report_general_questionnaire/action.php";
							            ...
							            <div class="col-lg-9">
							            	...
							            	<div class="table-responsive">
							                	<table class="table table-bordered">
							                		...
							                		<?php
							                		$data=mysqli_query($connect,"SELECT * FROM questionnaire_general as ql
							                		...
							                		?>
							                			...
							                			<?php
							                			$no=1;
								                		$data2=mysqli_query($connect,"SELECT * FROM questionnaire_general 
								                			WHERE category_id='$row[category_id]'");
								                		while ($row2=mysqli_fetch_array($data2)) {
								                			$satu=mysqli_num_rows(mysqli_query($connect,"SELECT * FROM questionnaire_report_general where 
								                				q_general_id='$row2[q_general_id]' AND 
								                				q_general_answer='1'"));

								                			$dua=mysqli_num_rows(mysqli_query($connect,"SELECT * FROM questionnaire_report_general where 
								                				q_general_id='$row2[q_general_id]' AND 
								                				q_general_answer='2'"));

								                			$tiga=mysqli_num_rows(mysqli_query($connect,"SELECT * FROM questionnaire_report_general where 
								                				q_general_id='$row2[q_general_id]' AND 
								                				q_general_answer='3'"));

								                			$empat=mysqli_num_rows(mysqli_query($connect,"SELECT * FROM questionnaire_report_general where 
								                				q_general_id='$row2[q_general_id]' AND 
								                				q_general_answer='4'"));

								                			$lima=mysqli_num_rows(mysqli_query($connect,"SELECT * FROM questionnaire_report_general where 
								                				q_general_id='$row2[q_general_id]' AND 
								                				q_general_answer='5'"));
								                		...
								                		<tr>
								                			<td><?= $no; ?></td>
								                			<td><?= $row2['q_general_description']; ?></td>
								                			<td><?= $satu; ?></td>
								                			<td><?= $dua; ?></td>
								                			<td><?= $tiga; ?></td>
								                			<td><?= $empat; ?></td>
