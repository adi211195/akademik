...
switch ($action) {			
		case 'input':
			...
			$save=mysqli_query($connect, "INSERT INTO forum_majors (
						forum_id,
						account_id,
						majors_code,
						forum_title,
		...
		case 'edit':
			...
			$update=mysqli_query($connect, "UPDATE forum_majors SET	
		...
		case 'remove':
			...
			$remove=mysqli_query($connect, "DELETE FROM forum_majors  
						where forum_id='$id'");
			if ($remove) {
				echo json_encode(array('status'=>'success'));
			} else {			
