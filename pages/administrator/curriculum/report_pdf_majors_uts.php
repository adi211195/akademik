...
<body>
	...
	<p align="center"><font size="18px"><b>Jadwal Ujian Tengah Semester (UTS)<br>
						<?= $code['majors']; ?> <br>
						<?= $sy; ?> - <?= $sm; ?></b></font></p>
	...
	<tbody>
		...
		$data=mysqli_query($connect, "SELECT * FROM master_curriculum as mc
				...
				DAYNAME(mcuts.uts_date)='$day[day]'");
		...
		<tr>
			...
			<td><?= $row['uts_date']; ?>, <?= substr($row['uts_start'],0,5); ?> - <?= substr($row['uts_end'],0,5); ?></td>
		</tr>
		<?php $no++; } ?>
	</tbody>
	</table>
...
$dompdf->render();
$dompdf->stream('Majors UTS.pdf',array('Attachment'=>0));
