			<div id="content-container">
                <div id="page-head">
                    ...
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <div id="page-title">
                        <h1 class="page-header text-overflow">Questionnaire Advisor</h1>
                    </div>
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <!--End page title-->
					...
					<li class="active">Questionnaire Advisor</li>
                    </ol>
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <!--End breadcrumb-->
                ...
                $page_input="page.php?p=questionnaire_advisor&act=input";
                $page_detail="page.php?p=questionnaire_advisor&act=detail";
                $back="page.php?p=questionnaire_advisor";
                $action="pages/mahasiswa/questionnaire_advisor/action.php";
					            ...
					            <div class="panel">
					                ...
					                <div class="panel-body text-center">
					                    <div class="cls-content">
										    ...
										    <p class="h4 text-uppercase text-bold">Questionnaire advisor</p>
										    <?php
										    $qs=mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM questionnaire_status where questionnaire='advisor'"));

										    $answer=mysqli_fetch_array(mysqli_query($connect,"SELECT * FROM questionnaire_report_advisor
					                    			where school_year='$open_sy'
													AND semester='$open_sm'
													AND student_nim='$student_nim'
													AND qr_status='finish'"));

										    if (empty($answer['q_advisor_id'])) { 
				...
				$jml_answer=mysqli_num_rows(mysqli_query($connect,"SELECT * FROM questionnaire_advisor"));
				$tot_answer=mysqli_num_rows(mysqli_query($connect,"SELECT * FROM questionnaire_report_advisor	
													where school_year='$open_sy'
													AND semester='$open_sm'
													AND student_nim='$student_nim'"));
				...
				if ($sts=="questionnaire" OR empty($sts)) {
					$question=mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM questionnaire_advisor as qg
						INNER JOIN questionnaire_category as qc ON qc.category_id=qg.category_id
						limit $limit,1"));

					$answer=mysqli_fetch_array(mysqli_query($connect,"SELECT * FROM questionnaire_report_advisor
					                    			where q_advisor_id='$question[q_advisor_id]'
													AND school_year='$open_sy'
													AND semester='$open_sm'
													AND student_nim='$student_nim'"));


					if (empty($question['q_advisor_id'])) {
					...
					if ($answer['q_advisor_answer']=="1") {
						$satu="btn-danger";
					} elseif ($answer['q_advisor_answer']=="2") {
						$dua="btn-danger";
					} elseif ($answer['q_advisor_answer']=="3") {
						$tiga="btn-danger";
					} elseif ($answer['q_advisor_answer']=="4") {
						$empat="btn-danger";
					} elseif ($answer['q_advisor_answer']=="5") {
						$lima="btn-danger";
					} else {
						$satu="";
						$dua="";
					...
					$question=mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM questionnaire_suggestions where qs_status='advisor' limit $limit,1"));
					            ...
					            <div class="fixed-fluid">
					                <div class="fixed-md-200 pull-sm-left fixed-right-border">
					                    ...
					                    <ul class="list-inline">
					                    	<?php
					                    	$no=1;
					                    	$data=mysqli_query($connect , "SELECT * FROM questionnaire_advisor order by category_id asc");
					                    	while ($row=mysqli_fetch_array($data)) {
					                    		$answer=mysqli_fetch_array(mysqli_query($connect,"SELECT * FROM questionnaire_report_advisor
					                    			where q_advisor_id='$row[q_advisor_id]'
													AND school_year='$open_sy'
													AND semester='$open_sm'
													AND student_nim='$student_nim'"));
					                    	...
					                    	?>
					                        <li class="tag tag-sm"><a href="<?= $pages; ?>"><?= $no; ?>. <span class="badge badge-danger"><?= $answer['q_advisor_answer']; ?></span></a></li>
					                        <?php $no++; } ?>
					                    </ul>
					                    ...
					                    <ul class="list-inline">
					                        <?php
					                    	$no=1;
					                    	$data=mysqli_query($connect , "SELECT * FROM questionnaire_suggestions where qs_status='advisor'");
					                    	while ($row=mysqli_fetch_array($data)) {
					                    		$answer=mysqli_fetch_array(mysqli_query($connect,"SELECT * FROM questionnaire_report_suggestions
					                ...
					                <div class="fluid">
					                    ...
					                    <div style="overflow: auto; height: 200px;">
						                    ...
						                    <p><h2 class="text-center">
						                    	<?= $question['q_advisor_description']; ?>
						                    </h2>					                    	
						                    </p>
						                </div>
						                <hr>
					                    ...
					                    <div class="text-center pad-to">
					                        <div class="btn-group" id="question">
					                            <button class="btn btn-lg <?= $satu; ?>" value="<?= $question['q_advisor_id']; ?>" onclick="data_question(this.value,'1',<?= $student_nim; ?>);">
					                            	<i class="demo-pli-consulting icon-lg icon-fw"></i> 1</button>

					                            <button class="btn btn-lg <?= $dua; ?>" value="<?= $question['q_advisor_id']; ?>" onclick="data_question(this.value,'2',<?= $student_nim; ?>);">
					                            	<i class="demo-pli-consulting icon-lg icon-fw"></i> 2</button>

					                            <button class="btn btn-lg <?= $tiga; ?>" value="<?= $question['q_advisor_id']; ?>" onclick="data_question(this.value,'3',<?= $student_nim; ?>);">
					                            	<i class="demo-pli-consulting icon-lg icon-fw"></i> 3</button>

					                            <button class="btn btn-lg <?= $empat; ?>" value="<?= $question['q_advisor_id']; ?>" onclick="data_question(this.value,'4',<?= $student_nim; ?>);">
					                            	<i class="demo-pli-consulting icon-lg icon-fw"></i> 4</button>

					                            <button class="btn btn-lg <?= $lima; ?>" value="<?= $question['q_advisor_id']; ?>" onclick="data_question(this.value,'5',<?= $student_nim; ?>);">
