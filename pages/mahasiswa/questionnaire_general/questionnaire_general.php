			<div id="content-container">
                <div id="page-head">
                    ...
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <div id="page-title">
                        <h1 class="page-header text-overflow">Questionnaire General</h1>
                    </div>
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <!--End page title-->
					...
					<li class="active">Questionnaire General</li>
                    </ol>
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <!--End breadcrumb-->
                ...
                $page_input="page.php?p=questionnaire_general&act=input";
                $page_detail="page.php?p=questionnaire_general&act=detail";
                $back="page.php?p=questionnaire_general";
                $action="pages/mahasiswa/questionnaire_general/action.php";
					            ...
					            <div class="panel">
					                ...
					                <div class="panel-body text-center">
					                    <div class="cls-content">
										    ...
										    <p class="h4 text-uppercase text-bold">Questionnaire General</p>
										    <?php
										    $qs=mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM questionnaire_status where questionnaire='general'"));

										    $answer=mysqli_fetch_array(mysqli_query($connect,"SELECT * FROM questionnaire_report_general
					                    			where school_year='$open_sy'
													AND semester='$open_sm'
													AND student_nim='$student_nim'
													AND qr_status='finish'"));

										    if (empty($answer['q_general_id'])) { 
				...
				if ($sts=="questionnaire" OR empty($sts)) {
					$question=mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM questionnaire_general as qg
						INNER JOIN questionnaire_category as qc ON qc.category_id=qg.category_id
						limit $limit,1"));

					$answer=mysqli_fetch_array(mysqli_query($connect,"SELECT * FROM questionnaire_report_general
					                    			where q_general_id='$question[q_general_id]'
													AND school_year='$open_sy'
													AND semester='$open_sm'
													AND student_nim='$student_nim'"));


					if (empty($question['q_general_id'])) {
					...
					if ($answer['q_general_answer']=="1") {
						$satu="btn-danger";
					} elseif ($answer['q_general_answer']=="2") {
						$dua="btn-danger";
					} elseif ($answer['q_general_answer']=="3") {
						$tiga="btn-danger";
					} elseif ($answer['q_general_answer']=="4") {
						$empat="btn-danger";
					} elseif ($answer['q_general_answer']=="5") {
						$lima="btn-danger";
					} else {
						$satu="";
						$dua="";
					...
					$question=mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM questionnaire_suggestions where qs_status='general' limit $limit,1"));
					            ...
					            <div class="fixed-fluid">
					                <div class="fixed-md-200 pull-sm-left fixed-right-border">
					                    ...
					                    <ul class="list-inline">
					                    	<?php
					                    	$no=1;
					                    	$data=mysqli_query($connect , "SELECT * FROM questionnaire_general order by category_id asc");
					                    	while ($row=mysqli_fetch_array($data)) {
					                    		$answer=mysqli_fetch_array(mysqli_query($connect,"SELECT * FROM questionnaire_report_general
					                    			where q_general_id='$row[q_general_id]'
													AND school_year='$open_sy'
													AND semester='$open_sm'
													AND student_nim='$student_nim'"));
					                    	...
					                    	?>
					                        <li class="tag tag-sm"><a href="<?= $pages; ?>"><?= $no; ?>. <span class="badge badge-danger"><?= $answer['q_general_answer']; ?></span></a></li>
					                        <?php $no++; } ?>
					                    </ul>
					                    ...
					                    <ul class="list-inline">
					                        <?php
					                    	$no=1;
					                    	$data=mysqli_query($connect , "SELECT * FROM questionnaire_suggestions where qs_status='general'");
					                    	while ($row=mysqli_fetch_array($data)) {
					                    		$answer=mysqli_fetch_array(mysqli_query($connect,"SELECT * FROM questionnaire_report_suggestions
					                    ...
					                    <?php 
					                    $jml_answer=mysqli_num_rows(mysqli_query($connect,"SELECT * FROM questionnaire_general"));
										$tot_answer=mysqli_num_rows(mysqli_query($connect,"SELECT * FROM questionnaire_report_general	
													where school_year='$open_sy'
													AND semester='$open_sm'
													AND student_nim='$student_nim'"));
					                ...
					                <div class="fluid">
					                    ...
					                    <div style="overflow: auto; height: 200px;">
						                    ...
						                    <p><h2 class="text-center">
						                    	<?= $question['q_general_description']; ?>
						                    </h2>					                    	
						                    </p>
						                </div>
						                <hr>
					                    ...
					                    <div class="text-center pad-to">
					                        <div class="btn-group" id="question">
					                            <button class="btn btn-lg <?= $satu; ?>" value="<?= $question['q_general_id']; ?>" onclick="data_question(this.value,'1',<?= $student_nim; ?>);">
					                            	<i class="demo-pli-consulting icon-lg icon-fw"></i> 1</button>

					                            <button class="btn btn-lg <?= $dua; ?>" value="<?= $question['q_general_id']; ?>" onclick="data_question(this.value,'2',<?= $student_nim; ?>);">
					                            	<i class="demo-pli-consulting icon-lg icon-fw"></i> 2</button>

					                            <button class="btn btn-lg <?= $tiga; ?>" value="<?= $question['q_general_id']; ?>" onclick="data_question(this.value,'3',<?= $student_nim; ?>);">
					                            	<i class="demo-pli-consulting icon-lg icon-fw"></i> 3</button>

					                            <button class="btn btn-lg <?= $empat; ?>" value="<?= $question['q_general_id']; ?>" onclick="data_question(this.value,'4',<?= $student_nim; ?>);">
					                            	<i class="demo-pli-consulting icon-lg icon-fw"></i> 4</button>

					                            <button class="btn btn-lg <?= $lima; ?>" value="<?= $question['q_general_id']; ?>" onclick="data_question(this.value,'5',<?= $student_nim; ?>);">
