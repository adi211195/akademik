...
switch ($action) {			
		case 'input':
			...
			$save=mysqli_query($connect, "INSERT INTO polling_majors (
						polling_id,
						account_id,
						majors_code,
						polling_title,
		...
		case 'edit':
			...
			$update=mysqli_query($connect, "UPDATE polling_majors SET	
		...
		case 'remove':
			...
			$remove=mysqli_query($connect, "DELETE FROM polling_majors  
						where polling_id='$id'");
			if ($remove) {
				echo json_encode(array('status'=>'success'));
			} else {			
